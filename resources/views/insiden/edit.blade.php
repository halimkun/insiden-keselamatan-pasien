                    ...
                    <div class="w-full">
                        ...
                        <div class="mt-6">
                            ...
                            <div class="flex gap-4">
                                <x-text-input class="flex-2" id="no_rekam_medis" type="text" name="no_rekam_medis" label="No. Rekam Medis" value="{{ old('no_rekam_medis', $insiden?->pasien?->no_rekam_medis) }}" readonly placeholder="No. Rekam Medis" />
                                <x-text-input class="flex-1" id="nama" type="text" name="nama" label="Nama" value="{{ old('nama', $insiden?->pasien?->nama) }}" readonly placeholder="Nama" />

                                <x-input-error class="mt-2" :messages="$errors->get('no_rekam_medis')" />
                            </div>
                            <div class="flex gap-4 mt-4">
                                <x-text-input class="flex-2" id="dob" type="text" name="dob" label="DOB" value="{{ old('tanggal_lahir', $insiden?->pasien?->tanggal_lahir->format('Y-m-d')) }}" readonly placeholder="DOB" />
                                <x-text-input class="flex-1" id="dob" type="text" name="dob" label="DOB" value="{{ old('tanggal_lahir', $insiden?->pasien?->tanggal_lahir->diff(\Carbon\Carbon::now())->format('%y Tahun %m Bulan %d Hari')) }}" readonly placeholder="DOB" />
                                <x-text-input class="flex-1" id="gender" type="text" name="gender" label="Gender" value="{{ old('jenis_kelamin', $insiden?->pasien?->jenis_kelamin == 'L' ? 'Laki-laki' : 'Perempuan') }}" readonly placeholder="gender" />
                            </div>
